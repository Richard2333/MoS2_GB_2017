...
module modperiod
  use modq
  use MatOps
  integer ,parameter :: ndos=255,ndv=20,nld=5,nrd=5
contains
  ...
  subroutine surf(g0, n, hm0, v, z)
    ...
    do k=1,100
			...
			do i=1,3
				do j=1,3
					call insertblock(n,3,g01,g0*expr((j-i)*kp),i,j)
				end do
			end do 
			hm1=0
...
program main
  ...
  complex,parameter :: ep1(3,3)=reshape([1.046,0.0,0.0,0.0,2.104,0.0,0.0,0.0,2.104],[3,3])
	...
	real,parameter :: ehgh=3.66,elow=-1.0
	...
	hblks(2,:,:)=h02
	hblks(3,:,:)=h03
	hblks(-1,:,:)=transpose(h01)
	hblks(-2,:,:)=transpose(h02)
	hblks(-3,:,:)=transpose(h03)
...
!~   rewind(235)
  ...
  hdfct=0
  do i=1,ndv
    read(234,*) ttmp,(tn1(j),j=1,ndv),rmved(i)
    do j=1,ndv
				if(tn1(j)/=0) call addblock(3,ndv,ndv,hdfct,hblks(tn1(j), :, :),i,j)
			end do
  end do
	z=(0,0)
