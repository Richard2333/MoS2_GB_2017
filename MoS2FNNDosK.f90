...
subroutine dosMo(z,dosi,hmdv,hmdpd,hmrd,hmrdpd,hmrdcp,hmrdcp1,hmld,hmldpd,hmldcp,hmldcp1,&
&hmlcp,hmlcp1,hmrcp,hmrcp1,hdfct,kk,rmved,kdos)
  ...
  complex :: sfedv(ndv*3,ndv*3),selfeg(ndv*3,ndv*3)
	...
	&,hmldcp1(nld*3,nld*3)
	complex :: hmlcp(ndv*3,nld*3),hmlcp1(ndv*3,nld*3)
	complex :: hmrcp(ndv*3,nrd*3),hmrcp1(ndv*3,nrd*3)
...
!~   	write(*,*) kp
    ...
    call surf(gl,nld*3,hamkl,hmldcp,z)!给定k得到lead自能
		...
		!得出左右lead耦合的作用（自能）
		hmlcp=hmlcp+hmlcp1*eppx
		hmrcp=hmrcp+hmrcp1*eppx
...
program main
	...
	complex :: hmlcp(ndv*3,nld*3),hmlcp1(ndv*3,nld*3)
	complex :: hmrcp(ndv*3,nrd*3),hmrcp1(ndv*3,nld*3)
	complex :: hdfct(3*ndv,3*ndv)
