...
module modperiod
  use modq
    integer,parameter :: ndv=20,nrd=10,nld=10,n=ndv,ndos=144
...
subroutine dosMo(z,dosi,hmdv,hmdpd,hmrd,hmrdpd,hmrdcp,hmld,hmldpd,hmldcp,hmlcp,hmrcp,hdfct,kk,rmved,kdos)
!~   use modperiod
  implicit none
  real,parameter :: pi1=-3.1416*2
...
!~ 		write(*,*) i,z
		...
		call  dosMo(z(i),dosi(i),hmdv,hmdpd,hmrd,hmrdpd,hmrdcp,hmld,hmldpd,hmldcp,hmlcp,hmrcp,hdfct,kk,rmved,kdos(i,:))
	end do
	open(unit=49,file='densos.csv')
	open(unit=44,file='kdensos.csv')
	do i=1,ndos
		write(49,*) real(z(i)),',',dosi(i)
		write(44,*) (kdos(i,j),',',j=1,nk)
	end do
