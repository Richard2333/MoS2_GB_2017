...
subroutine dosMo(z,dosi,hmdv,hmdpd,hmrd,hmrdpd,hmrdcp,hmld,hmldpd,hmldcp,hmlcp,hmrcp,hdfct,kk,rmved)
  use modperiod
  implicit none
  real,parameter :: pi1=-3.1416*2
  integer :: i,j,k,tmps(3),ttmp
  integer,parameter :: ndv=59,nrd=14,nld=14,n=ndv
  real :: kk(nk,2),kp,dosi
...
!~   	write(*,*) kp
	...
	eppx=exp((0,pi1)*kp)
		...
		hmrdcp=hmrdcp+hmrdcp1*eppx+hmrdcp2*conjg(eppx)
...
program main
	...
	integer :: i,j,k
	integer,parameter :: ndos=144,ndv=59,nld=14,nrd=14
	complex,parameter :: t=(-2.8,0)
	integer :: tn1(6),tn2(6),tn3(6),ttmp,rmved(ndv)
	real :: dosi(ndos)
	  real :: kk(nk,2)
	  complex :: z(ndos)
    complex :: h1(3,3),h2(3,3),h3(3,3)
	complex :: hmdv(ndv*3,ndv*3),hmdpd(ndv*3,ndv*3)
	complex :: hmrd(nrd*3,nrd*3),hmrdpd(nrd*3,nrd*3),hmrdcp(nrd*3,nrd*3)&
	&,hmrdcp1(nrd*3,nrd*3),hmrdcp2(nrd*3,nrd*3)
	complex :: hmld(nld*3,nld*3),hmldpd(nld*3,nld*3),hmldcp(nld*3,nld*3)&
	...
	do k=1,1
		do i=1,ndv
			read(233,*) ttmp,(tn1(j),j=1,6),(tn2(j),j=1,6),(tn3(j),j=1,6)
			do j=1,6
				if(tn1(j)/=0) call addblock(3,ndv,hmdv,h1,i,tn1(j))
				if(tn2(j)/=0) call addblock(3,ndv,hmdv,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmdv,h3,i,tn3(j))
			end do
		end do
		hmdpd=0
		do i=1,ndv
			read(233,*) ttmp,(tn1(j),j=1,6),(tn2(j),j=1,6),(tn3(j),j=1,6)
			do j=1,6
				if(tn1(j)/=0) call addblock(3,ndv,hmdpd,h1,i,tn1(j))
				if(tn2(j)/=0) call addblock(3,ndv,hmdpd,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmdpd,h3,i,tn3(j))
			end do
		end do
		...
		do i=1,nrd
			...
			do j=1,6
				...
				if(tn2(j)/=0) call addblock(3,ndv,hmrd,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmrd,h3,i,tn3(j))
			end do
		end do 
		hmrdpd=0
		do i=1,nrd
			...
			do j=1,6
				...
				if(tn2(j)/=0) call addblock(3,ndv,hmrdpd,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmrdpd,h3,i,tn3(j))
			end do
		end do 
		hmrdcp=0
		do i=1,nrd
			...
			do j=1,6
				...
				if(tn2(j)/=0) call addblock(3,ndv,hmrdcp,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmrdcp,h3,i,tn3(j))
			end do
		end do
		hmrdcp1=0!cpl：耦合上一层
		do i=1,nrd
			...
			do j=1,6
				...
				if(tn2(j)/=0) call addblock(3,ndv,hmrdcp1,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmrdcp1,h3,i,tn3(j))
			end do
		end do
		hmrdcp2=0!cpl：耦合上一层
		do i=1,nrd
			read(233,*) ttmp,(tn1(j),j=1,6),(tn2(j),j=1,6),(tn3(j),j=1,6)
			do j=1,6
				if(tn1(j)/=0) call addblock(3,ndv,hmrdcp2,h1,i,tn1(j))
				if(tn2(j)/=0) call addblock(3,ndv,hmrdcp2,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmrdcp2,h3,i,tn3(j))
			end do
		end do  
		...
		do i=1,nld
			...
			do j=1,6
				...
				if(tn2(j)/=0) call addblock(3,ndv,hmld,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmld,h3,i,tn3(j))
			end do
		end do 
		hmldpd=0
		do i=1,nld
			...
			do j=1,6
				...
				if(tn2(j)/=0) call addblock(3,ndv,hmldpd,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmldpd,h3,i,tn3(j))
			end do
		end do 
		hmldcp=0
		do i=1,nld
			...
			do j=1,6
				...
				if(tn2(j)/=0) call addblock(3,ndv,hmldcp,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmldcp,h3,i,tn3(j))
			end do
		end do
		hmldcp1=0!cpl：耦合上一层
		do i=1,nld
			...
			do j=1,6
				...
				if(tn2(j)/=0) call addblock(3,ndv,hmldcp1,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmldcp1,h3,i,tn3(j))
			end do
		end do
		hmldcp2=0!cpl：耦合上一层
		do i=1,nld
			read(233,*) ttmp,(tn1(j),j=1,6),(tn2(j),j=1,6),(tn3(j),j=1,6)
			do j=1,6
				if(tn1(j)/=0) call addblock(3,ndv,hmldcp2,h1,i,tn1(j))
				if(tn2(j)/=0) call addblock(3,ndv,hmldcp2,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmldcp2,h3,i,tn3(j))
			end do
		end do  
		...
		do i=1,nrd
			read(233,*) ttmp,(tn1(j),j=1,6),(tn2(j),j=1,6),(tn3(j),j=1,6)
			do j=1,6
				if(tn1(j)/=0) call addblock(3,ndv,hmrcp,h1,i,tn1(j))
				if(tn2(j)/=0) call addblock(3,ndv,hmrcp,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmrcp,h3,i,tn3(j))
			end do
		end do
		hmrcp1=0
		do i=1,nrd
			read(233,*) ttmp,(tn1(j),j=1,6),(tn2(j),j=1,6),(tn3(j),j=1,6)
			do j=1,6
				if(tn1(j)/=0) call addblock(3,ndv,hmrcp1,h1,i,tn1(j))
				if(tn2(j)/=0) call addblock(3,ndv,hmrcp1,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmrcp1,h3,i,tn3(j))
			end do
		end do
		hmrcp2=0
		do i=1,nrd
			read(233,*) ttmp,(tn1(j),j=1,6),(tn2(j),j=1,6),(tn3(j),j=1,6)
			do j=1,6
				if(tn1(j)/=0) call addblock(3,ndv,hmrcp2,h1,i,tn1(j))
				if(tn2(j)/=0) call addblock(3,ndv,hmrcp2,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmrcp2,h3,i,tn3(j))
			end do
		end do
		...
		do i=1,nld
			read(233,*) ttmp,(tn1(j),j=1,6),(tn2(j),j=1,6),(tn3(j),j=1,6)
			do j=1,6
				if(tn1(j)/=0) call addblock(3,ndv,hmlcp,h1,i,tn1(j))
				if(tn2(j)/=0) call addblock(3,ndv,hmlcp,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmlcp,h3,i,tn3(j))
			end do
		end do
		hmlcp1=0
		do i=1,nld
			read(233,*) ttmp,(tn1(j),j=1,6),(tn2(j),j=1,6),(tn3(j),j=1,6)
			do j=1,6
				if(tn1(j)/=0) call addblock(3,ndv,hmlcp1,h1,i,tn1(j))
				if(tn2(j)/=0) call addblock(3,ndv,hmlcp1,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmlcp1,h3,i,tn3(j))
			end do
		end do
		hmlcp2=0
		do i=1,nld
			read(233,*) ttmp,(tn1(j),j=1,6),(tn2(j),j=1,6),(tn3(j),j=1,6)
			do j=1,6
				if(tn1(j)/=0) call addblock(3,ndv,hmlcp2,h1,i,tn1(j))
				if(tn2(j)/=0) call addblock(3,ndv,hmlcp2,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmlcp2,h3,i,tn3(j))
			end do
		end do
...
!~   rewind(235)
  ...
  hdfct=0
  do i=1,ndv
    read(234,*) ttmp,(tn1(j),j=1,6),(tn2(j),j=1,6),(tn3(j),j=1,6),rmved(i)
    do j=1,6
				if(tn1(j)/=0) call addblock(3,ndv,hmlcp2,h1,i,tn1(j))
				if(tn2(j)/=0) call addblock(3,ndv,hmlcp2,h2,i,tn2(j))
				if(tn3(j)/=0) call addblock(3,ndv,hmlcp2,h3,i,tn3(j))
...
!~ 		write(*,*) i,z
		...
		call  dos(z(i),dosi(i),hmdv,hmdpd,hmrd,hmrdpd,hmrdcp,hmld,hmldpd,hmldcp,hmlcp,hmrcp,hdfct,kk,rmved)
	end do
